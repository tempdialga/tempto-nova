...
public class WorldMap {
    ...
    public WorldMap(String mapID){
        ...
        boolean hasExistingData = Gdx.files.local("data/maps/" + mapID + ".dat").exists();
        if (hasExistingData) {
            //Load using existing data where appropriate
        } else {
            //Load using map file defaults
    ...
    class PersistentMapData {
        /**Filepath for the Tiled map file (.tmx). If not otherwise specified, the loader will look for an identically named JSON (extension .dat) file in the local file directory*/
        String tiledMapFilePath;
        /**Specifies whether the base tmx file is located internally. True by default, should rarely if ever need to be local or other.*/
        boolean fileInternal;
        /**An array list of entities in the map, stored in serialized form. Entity classes determine how their data is serialized; Classes which do not specify will be serialized by gdx library {@link Json}*/
