...
void addShadowFromS(in vec3 S, in vec3 T,
in vec3 a, in vec3 ab, in vec3 ac,
in float t_mod,
inout vec4 shadowColor);
...
//    }
    ...
    addShadowFromS(S_a, T, a, ab, ac, t_mod, shadColor);
    vec3 S_b = vec3(S.x-w_r, S.y+w_r, S.z);
    addShadowFromS(S_b, T, a, ab, ac, t_mod, shadColor);
    vec3 S_c = vec3(S.x+w_r, S.y+w_r, S.z);
    addShadowFromS(S_c, T, a, ab, ac, t_mod, shadColor);
    vec3 S_d = vec3(S.x+w_r, S.y-w_r, S.z);
    addShadowFromS(S_d, T, a, ab, ac, t_mod, shadColor);

    shadColor *= 0.25;

    gl_FragColor = shadColor;
