...
//    float t_mod = (3*(shadDep.r-0.5))/abs(lST.z);
    ...
    float d_00 = depth_mod_max*(Dmap_00.r-0.5);
    float t_mod_00 = t_mod_base-d_00/abs(S_00.z-T.z);
    if (!intersectionValid(uvt_00, t_mod_00)) C_00 = vec4(0);
    ...
    float d_0m = depth_mod_max*(Dmap_0m.r-0.5);
    float t_mod_0m = t_mod_base-d_0m/abs(S_0m.z-T.z);
    if (!intersectionValid(uvt_0m, t_mod_0m)) C_0m = vec4(0);
    ...
    float d_01 = depth_mod_max*(Dmap_01.r-0.5);
    float t_mod_01 = t_mod_base-d_01/abs(S_01.z-T.z);
    if (!intersectionValid(uvt_00, t_mod_01)) C_01 = vec4(0);
    ...
    float d_m0 = depth_mod_max*(Dmap_m0.r-0.5);
    float t_mod_m0 = t_mod_base-d_m0/abs(S_m0.z-T.z);
    if (!intersectionValid(uvt_m0, t_mod_m0)) C_m0 = vec4(0);
    ...
    float d_mm = depth_mod_max*(Dmap_mm.r-0.5);
    float t_mod_mm = t_mod_base-d_mm/abs(S_mm.z-T.z);
    vec4 C_mm = texture2D(u_shadTex, v_shadUV + v_shadWH*vec2(uvt_mm.x, 1-uvt_mm.y));
    if (!intersectionValid(uvt_mm, t_mod_mm)) C_mm = vec4(0);
    ...
    float d_m1 = depth_mod_max*(Dmap_m1.r-0.5);
    float t_mod_m1 = t_mod_base-d_m1/abs(S_m1.z-T.z);
    if (!intersectionValid(uvt_m0, t_mod_m1)) C_m1 = vec4(0);
    ...
    float d_10 = depth_mod_max*(Dmap_10.r-0.5);
    float t_mod_10 = t_mod_base-d_10/abs(S_10.z-T.z);
    if (!intersectionValid(uvt_10, t_mod_10)) C_10 = vec4(0);
    ...
    float d_1m = depth_mod_max*(Dmap_1m.r-0.5);
    float t_mod_1m = t_mod_base-d_1m/abs(S_1m.z-T.z);
    if (!intersectionValid(uvt_1m, t_mod_1m)) C_1m = vec4(0);
    ...
    float d_11 = depth_mod_max*(Dmap_11.r-0.5);
    float t_mod_11 = t_mod_base-d_11/abs(S_11.z-T.z);
    if (!intersectionValid(uvt_10, t_mod_11)) C_11 = vec4(0);
